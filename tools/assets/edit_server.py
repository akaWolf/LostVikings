...
def logic_page_lvd(cid_hex):
    ...
    for line in text.splitlines():
        ...
        if raw.startswith(("state ", "func ", "loop ", "anim ", "palette ")) and raw.endswith(":"):
            ...
            esc2 = _re.sub(r"\b(goto|call|-&gt;|anim|palette) ([A-Za-z_][A-Za-z0-9_]*)", r"\1 <a href='#\2'>\2</a>", esc)   # goto/call/anim targets and switch cases
    ...
    return ("<!doctype html><meta charset='utf-8'><title>logic " + cid_hex + " .lvd</title>"
            ...
            + "".join(out) + "</div>"
            "<div id=right><b>edit .lvd</b> <span class=c>(one statement per line, 4-space indent; `state name:` / `func name:` / `anim name:` / `palette name:` labels, inner labels of a func `  name:`; the left operand of a comparison is the accumulator load; compile lowers to .lvsf and assembles)</span>"
