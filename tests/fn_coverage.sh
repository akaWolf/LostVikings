...
# Использование:
#   COV_SEG000=1 TEST=1 HEADLESS=1 make -j$(nproc)   # инструментированная сборка
#   bash tests/fn_coverage.sh                  # прогон + отчёт
set -u
cd "$(dirname "$0")/.."
BIN=./vikings_headless
[ -x "$BIN" ] || { echo "no $BIN"; exit 1; }
[ -f .obj-headless-test/src/vikings.exe_seg000.gcno ] || {
  echo "no .gcno — build with COV_SEG000=1 TEST=1 HEADLESS=1 make"; exit 1; }
rm -f .obj-headless-test/src/vikings.exe_seg000.gcda
...
echo "units run: $n"
gcov -b --json-format -o .obj-headless-test/src src/vikings.exe_seg000.cpp \
  > /dev/null 2>&1
