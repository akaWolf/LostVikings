...
extern uint32_t RGBtoYUV[32768];   /* Lost Vikings: RGB555-indexed (see NOTES.txt) */

static inline uint32_t rgb_to_yuv(uint32_t c)
{
    // Mask against MASK_RGB to discard the alpha channel
    return RGBtoYUV[((c >> 9) & 0x7C00) | ((c >> 6) & 0x03E0) | ((c >> 3) & 0x001F)];   /* Lost Vikings: 5 bits per channel */
}
