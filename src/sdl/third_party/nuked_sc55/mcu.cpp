...
#include "lcd.h"
#include "submcu.h"
#ifndef NUKED_SC55_EMBEDDED   // the standalone front end only (Lost Vikings embeds the core: v2_sc55.cpp drives it)
#include "midi.h"
#include "utf8main.h"
...
#ifndef NUKED_SC55_EMBEDDED   // the standalone main loop (LCD window)
static void MCU_Run()
{
    bool working = true;
...
#ifndef NUKED_SC55_EMBEDDED   // the standalone SDL audio device
...
#ifndef NUKED_SC55_EMBEDDED   // the standalone: ROM files by argv, MIDI input, main()
...
// ---------------------------------------------------------------------------
// Lost Vikings embedding (src/sdl/v2_sc55.cpp): the same core, driven without
// the standalone's window, audio device and MIDI input. The ROM set is
// autodetected in a directory the way main() does it above (the first set
// whose files are all present), the sample ring is read by the game's own
// audio mixer at the device's pace (as the SDL callback above did — the
// work thread paces itself against the read pointer), MIDI bytes come from
// the game's AIL driver through MCU_PostUART.
// ---------------------------------------------------------------------------
#include <string>
static SDL_Thread* nsc55_thread = nullptr;
static bool nsc55_loaded = false;
static char nsc55_error[256] = "";

extern "C" const char* nsc55_last_error(void) { return nsc55_error; }
extern "C" const char* nsc55_model_name(void) { return rs_name[romset]; }
extern "C" int nsc55_sample_rate(void) { return (mcu_mk1 || mcu_jv880) ? 64000 : 66207; }

static bool nsc55_read_file(const std::string& path, uint8_t* dst, size_t len, bool must)
{
    FILE* f = fopen(path.c_str(), "rb");
    if (!f) { if (must) snprintf(nsc55_error, sizeof nsc55_error, "missing %s", path.c_str()); return false; }
    size_t n = fread(dst, 1, len, f); fclose(f);
    if (n != len) { snprintf(nsc55_error, sizeof nsc55_error, "%s: %zu of %zu bytes", path.c_str(), n, len); return false; }
    return true;
}

// 1 = loaded (romset set, the model flags set, every image read and unscrambled), 0 = no usable set in `dir`
extern "C" int nsc55_load(const char* dir)
{
    std::string base = dir ? dir : ".";
    int found = -1;
    for (size_t i = 0; i < ROM_SET_COUNT && found < 0; i++) {
        bool good = true;
        for (size_t j = 0; j < 5; j++) {
            if (roms[i][j][0] == '\0') continue;
            FILE* h = fopen((base + "/" + roms[i][j]).c_str(), "rb");
            if (!h) { good = false; break; }
            fclose(h);
        }
        if (good) found = (int)i;
    }
    if (found < 0) { snprintf(nsc55_error, sizeof nsc55_error, "no SC-55 ROM set in %s (e.g. rom1.bin rom2.bin waverom1.bin waverom2.bin rom_sm.bin for the SC-55mk2)", base.c_str()); return 0; }
    romset = found;
    mcu_mk1 = false; mcu_cm300 = false; mcu_st = false; mcu_jv880 = false; mcu_scb55 = false; mcu_sc155 = false;
    rom2_mask = ROM2_SIZE - 1;
    switch (romset) {
        case ROM_SET_MK2: case ROM_SET_SC155MK2: if (romset == ROM_SET_SC155MK2) mcu_sc155 = true; break;
        case ROM_SET_ST: mcu_st = true; break;
        case ROM_SET_MK1: case ROM_SET_SC155: mcu_mk1 = true; mcu_st = false; if (romset == ROM_SET_SC155) mcu_sc155 = true; break;
        case ROM_SET_CM300: mcu_mk1 = true; mcu_cm300 = true; break;
        case ROM_SET_JV880: mcu_jv880 = true; rom2_mask /= 2; break;
        case ROM_SET_SCB55: case ROM_SET_RLP3237: mcu_scb55 = true; break;
    }
    std::string rp[ROM_SET_N_FILES];
    for (size_t i = 0; i < ROM_SET_N_FILES; ++i) rp[i] = roms[romset][i][0] ? base + "/" + roms[romset][i] : "";
    memset(&mcu, 0, sizeof(mcu_t));
    if (!nsc55_read_file(rp[0], rom1, ROM1_SIZE, true)) return 0;
    {
        FILE* f = fopen(rp[1].c_str(), "rb");
        if (!f) { snprintf(nsc55_error, sizeof nsc55_error, "missing %s", rp[1].c_str()); return 0; }
        size_t rom2_read = fread(rom2, 1, ROM2_SIZE, f); fclose(f);
        if (rom2_read == ROM2_SIZE || rom2_read == ROM2_SIZE / 2) rom2_mask = (int)rom2_read - 1;
        else { snprintf(nsc55_error, sizeof nsc55_error, "%s: unexpected size %zu", rp[1].c_str(), rom2_read); return 0; }
    }
    if (mcu_mk1) {
        if (!nsc55_read_file(rp[2], tempbuf, 0x100000, true)) return 0; unscramble(tempbuf, waverom1, 0x100000);
        if (!nsc55_read_file(rp[3], tempbuf, 0x100000, true)) return 0; unscramble(tempbuf, waverom2, 0x100000);
        if (!nsc55_read_file(rp[4], tempbuf, 0x100000, true)) return 0; unscramble(tempbuf, waverom3, 0x100000);
    } else if (mcu_jv880) {
        if (!nsc55_read_file(rp[2], tempbuf, 0x200000, true)) return 0; unscramble(tempbuf, waverom1, 0x200000);
        if (!nsc55_read_file(rp[3], tempbuf, 0x200000, true)) return 0; unscramble(tempbuf, waverom2, 0x200000);
        if (!rp[4].empty() && nsc55_read_file(rp[4], tempbuf, 0x800000, false)) unscramble(tempbuf, waverom_exp, 0x800000);
        if (!rp[5].empty() && nsc55_read_file(rp[5], tempbuf, 0x200000, false)) unscramble(tempbuf, waverom_card, 0x200000);
    } else {
        if (!nsc55_read_file(rp[2], tempbuf, 0x200000, true)) return 0; unscramble(tempbuf, waverom1, 0x200000);
        if (!rp[3].empty()) {
            if (!nsc55_read_file(rp[3], tempbuf, 0x100000, true)) return 0;
            unscramble(tempbuf, mcu_scb55 ? waverom3 : waverom2, 0x100000);
        }
        if (!rp[4].empty() && !nsc55_read_file(rp[4], sm_rom, ROMSM_SIZE, true)) return 0;
    }
    nsc55_loaded = true;
    return 1;
}
...
extern "C" int nsc55_start(int ring_frames)
{
    ...
    if (nsc55_thread) return 1;
    if (ring_frames < 1024) ring_frames = 1024;
    audio_page_size = 512;
    audio_buffer_size = ring_frames * 2;
    ...
    PCM_Reset();
    work_thread_run = true;
    nsc55_thread = SDL_CreateThread(work_thread, "sc55", 0);
    if (!nsc55_thread) { work_thread_run = false; snprintf(nsc55_error, sizeof nsc55_error, "cannot start the emulation thread"); return 0; }
    return 1;
}

extern "C" void nsc55_stop(void)
{
    if (!nsc55_thread) return;
    work_thread_run = false;
    // the work thread may be parked on a full ring: let it out
    sample_read_ptr = (sample_read_ptr + 2) % audio_buffer_size;
    SDL_WaitThread(nsc55_thread, 0);
    nsc55_thread = nullptr;
    free(sample_buffer); sample_buffer = nullptr;
}

// consume `frames` stereo frames at the device's pace (the standalone's
// callback: whatever the ring holds, zeros where the emulation lags); the
// work thread paces itself against the read pointer
extern "C" void nsc55_read(short* out, int frames)
{
    if (!nsc55_thread || !sample_buffer) { memset(out, 0, (size_t)frames * 2 * sizeof(short)); return; }
    for (int i = 0; i < frames * 2; i++) {
        out[i] = sample_buffer[sample_read_ptr];
        sample_buffer[sample_read_ptr] = 0;
        sample_read_ptr = (sample_read_ptr + 1) % audio_buffer_size;
    }
}
