...
void v2_options_ensure_loaded() {
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        if (sscanf(line, " %63[a-z_] = %d", key, &val) == 2) {
            ...
            else if (!strcmp(key, "music_volume")) v2_options.music_volume = (val >= 0 && val <= 100) ? val : 100;
            else if (!strcmp(key, "sfx_volume")) v2_options.sfx_volume = (val >= 0 && val <= 100) ? val : 100;
        }
    }
    fclose(f);
}
...
bool v2_ui_draw(uint32_t* rgba, int w, int h, SDL_PixelFormat* fmt, bool transparent) {
    ...
    if (v2_ui_menu_open.load()) {
        ...
        snprintf(lines[n++], 44, "CAMERA    < %s >%s", v2_options.camera.load() ? "SMOOTH  " : "ORIGINAL",
                 (v2_options.camera.load() && !v2_options.subpixel.load()) ? "  (NEEDS SUBPIXEL)" : "");
        ...
        snprintf(lines[n++], 40, "MUSIC VOL < %3d%% >", v2_options.music_volume.load());
        snprintf(lines[n++], 40, "SFX VOL   < %3d%% >", v2_options.sfx_volume.load());
