...
- Обвязка (`v2_sc55.{h,cpp}`): старт на игровом потоке при выборе режима 2 (`v2_ui_service`:
  загрузка ROM-ов один раз, `nsc55_start`, GS-reset `F0 41 10 42 12 40 00 7F 00 41 F7`);
  без ROM-ов — тост со статусом и возврат режима на PC. `v2_sc55_midi` — статус + данные
  (2 байта для 0xC0/0xD0, иначе 3) в UART. `v2_sc55_mix` (аудио-поток, после SNES-микса в
